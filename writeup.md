...
As I grew older, these creativity-killing restrictions bugged me. What was the point of customization if it was so constrained?

This question was the reason I made this program. I wanted real customization, not the ability to buy a paint job. If I wanted to change its engine and observe the effects, I could. If I wanted to floor the virtual gas and get
...
def exhaust(self):
  pass
```
You'll notice that there is no function for compression. This is because, during the second stroke, the compression is a natural result of the piston pushing the gas vapor into a tighter area. There doesn't need to be an extra 
function because there is no special behavior happening.

I am no expert in chemistry or thermodynamics, so coding a realistic model of combustion seemed too far out of my scope. Instead, I decided to use the ideal gas law, which greatly simplified my task. The intake stroke would introduce some amount of air and fuel into the cylinder, increasing the number of moles and pressure. The compression stroke would decrease volume, raising the pressure and temperature. Then, the `spark()` function would raise the temperature based on the amount of gas injected, which would then increase the pressure in the cylinder.
...
To remedy this, I decided to focus on the next most important part of the engine: the crankshaft.
```
class Crankshaft
```
...
positions to the angle of the crankshaft using `cos(theta)`, and converted the force generated by each piston to torque using `sin(theta)`.
...
At last, we find ourselves nearing the end: the wheels (and the rest) of the car.
